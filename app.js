...
function checkpoint() {
  try { undoStack.push(snapshot()); if (undoStack.length > UNDO_MAX) undoStack.shift(); } catch {}
}
function undoLast() {
...
let statistiky = {};        // id -> struktura níže
let goloveUdalosti = [];    // {typ:"g"|"o", cas, tretina, strelec?, asistenti[], plus[], golman?, minus[]}
...
function sortHraci(list){
  const P = {B:0, O:1, Ú:2};
  return [...list].sort((a,b)=>(P[a.typ]-P[b.typ]) || ((a.petka||0)-(b.petka||0)) || cisloZJmena(a.jmeno).localeCompare(cisloZJmena(b.jmeno),"cs"));
}
...
// pouze Pointer Events – krátký klik = střela/zásah, dlouhý = trest
function attachPressHandlers(btn, h){
  ...
  const onPointerDown = (e) => {
    ...
    timer = setTimeout(()=>{ longFired = true; recordPenaltyFor(h); timer = null; }, LONG_MS);
  };
...
//////////////////// SCORE & SHOTS SUMMARIES ////////////////////
function souhrnSkore(){
  let dom=0, hos=0;
  const per = {"1":[0,0],"2":[0,0],"3":[0,0],"P":[0,0]};
  for (const e of goloveUdalosti){
    const domaci = infoZapasu.tym === "domaci";
    if (e.typ === "g")  domaci ? per[e.tretina][0]++ : per[e.tretina][1]++;
...
function souhrnStrely(){
  ...
  const domaci = infoZapasu.tym === "domaci";
  const dom = domaci ? sum(nasi) : sum(soup);
  const hos = domaci ? sum(soup) : sum(nasi);
  const per = ["1","2","3","P"].map(t=>{
    const d = domaci ? nasi[t] : soup[t];
    const h = domaci ? soup[t] : nasi[t];
    return `${d}:${h}`;
  }).join(";");
  return `${dom}:${hos} (${per})`;
}
...
function renderOverlay(){
  ...
  modes.className = "flex flex-wrap gap-2 mb-3";
  const chip = (key,label)=>{
    const b=document.createElement("button"); const act=overlay.selectMode===key;
    b.textContent=label; b.className=(act?"bg-black text-white":"bg-gray-200")+" px-3 py-1 rounded";
    b.onclick=()=>{ overlay.selectMode=key; removeOverlayDom(); render(); };
    return b;
  };
  if (overlay.mode==="g"){
    modes.appendChild(chip("shooter","Střelec"));
    modes.appendChild(chip("assist","Asistence (0–2)"));
    modes.appendChild(chip("plus","+ na ledě"));
  } else {
    modes.appendChild(chip("goalie","Brankář"));
    modes.appendChild(chip("minus","− na ledě"));
  }
  card.appendChild(modes);

  const bar=document.createElement("div"); bar.className="flex flex-wrap gap-2 mb-3";
  const bClear=document.createElement("button"); bClear.textContent="🧹 Vymazat volby"; bClear.className="px-3 py-1 rounded bg-gray-300";
  bClear.onclick=()=>{ if(overlay.mode==="g"){ overlay.shooter=null; overlay.A=new Set(); overlay.plus=new Set(); } else { overlay.goalie=null; overlay.minus=new Set(); } removeOverlayDom(); render(); };
  bar.appendChild(bClear);

  if (overlay.mode==="g"){
    const bNoA=document.createElement("button"); bNoA.textContent="💾 Uložit bez asistentů"; bNoA.className="px-3 py-1 rounded bg-green-600 text-white";
    bNoA.onclick=()=>{ const bak=overlay.A; overlay.A=new Set(); ulozOverlay(); overlay.A=bak; };
    bar.appendChild(bNoA);
  }
    ...
    btn.onclick=()=>{
      if (overlay.mode==="g"){
        if (overlay.selectMode === "shooter"){
          ...
          else if (overlay.A.size < 2) overlay.A.add(h.id);
        } else {
...
//////////////////// SHOOTOUTS (SO) ////////////////////
let soOverlay = null; // {view:"menu"|"us"|"opp", shooterId?, goalieId?}
function shootoutCounts(){ return { us: shootoutAttempts.filter(a=>a.team==="us").length, opp: shootoutAttempts.filter(a=>a.team==="opp").length }; }
function shootoutScore(){
  const us = shootoutAttempts.filter(a=>a.team==="us" && a.result==="goal").length;
  const opp = shootoutAttempts.filter(a=>a.team==="opp" && a.result==="goal").length;
  return {us, opp, rounds: Math.max(shootoutCounts().us, shootoutCounts().opp)};
}
...
function renderShootoutOverlay(){
  ...
  const close=document.createElement("button"); close.textContent="✕"; close.className="absolute right-2 top-2 px-2 py-1 bg-gray-200 rounded"; close.onclick=()=>closeShootoutOverlay(); card.appendChild(close);
  ...
  if(soOverlay.view==="menu"){
    ...
    g.onclick=()=>{ if(!soOverlay.shooterId){ alert("Vyber střelce."); return; } addShootoutAttempt({team:"us", shooterId:soOverlay.shooterId, result:"goal"}); };
    const m=document.createElement("button"); m.textContent="Neúspěch"; m.className="px-3 py-2 rounded bg-gray-600 text-white";
    m.onclick=()=>{ if(!soOverlay.shooterId){ alert("Vyber střelce."); return; } addShootoutAttempt({team:"us", shooterId:soOverlay.shooterId, result:"miss"}); };
    ...
    g.onclick=()=>{ if(!soOverlay.goalieId){ alert("Vyber gólmana."); return; } addShootoutAttempt({team:"opp", goalieId:soOverlay.goalieId, result:"goal"}); };
    const s=document.createElement("button"); s.textContent="Zákrok (save)"; s.className="px-3 py-2 rounded bg-green-700 text-white";
    s.onclick=()=>{ if(!soOverlay.goalieId){ alert("Vyber gólmana."); return; } addShootoutAttempt({team:"opp", goalieId:soOverlay.goalieId, result:"save"}); };
    const back=document.createElement("button"); back.textContent="Zpět do menu"; back.className="px-3 py-2 rounded bg-gray-300"; back.onclick=()=>{ soOverlay={view:"menu"}; refreshSO(); };
...
function renderHlavicka(){
  ...
  const line=document.createElement("div"); line.className="flex flex-wrap gap-3 items-end";
  ...
  dWrap.appendChild(dLbl); dWrap.appendChild(dSel); line.appendChild(dWrap);
  ...
  hraci.filter(h=>h.active!==false && h.typ==="B").forEach(g=>{
    const o=document.createElement("option"); o.value=g.id; o.textContent=`#${cisloZJmena(g.jmeno)} ${jmenoBezCisla(g.jmeno)}`; if(activeGoalieId===g.id) o.selected=true; gSel.appendChild(o);
  });
  gSel.onchange=()=>{ activeGoalieId=gSel.value||null; saveState(); render(); };
  gWrap.appendChild(gLbl); gWrap.appendChild(gSel); line.appendChild(gWrap);

  // Actions
  const actions=document.createElement("div"); actions.className="flex gap-2 ml-auto items-end";

  const bUndo=document.createElement("button"); bUndo.textContent="↩︎ Zpět"; bUndo.className="px-3 py-1 rounded bg-gray-700"; bUndo.disabled=undoStack.length===0; bUndo.onclick=undoLast;
  actions.appendChild(bUndo);

  const bSettings=document.createElement("button"); bSettings.innerHTML="⚙️"; bSettings.title="Nastavení"; bSettings.className=(showSettings?"bg-blue-700":"bg-gray-700")+" px-3 py-1 rounded"; bSettings.onclick=()=>{ showSettings=!showSettings; saveState(); render(); };
  actions.appendChild(bSettings);

  const bNew=document.createElement("button"); bNew.textContent="🆕 Nový zápas"; bNew.className="px-3 py-1 rounded bg-gray-700";
  bNew.onclick=()=>{ if(!confirm("Smazat statistiky a začít nový zápas? Soupiska zůstane.")) return; checkpoint(); goloveUdalosti=[]; resetStatistik(); zamknuto=false; aktivniPetka=0; activeGoalieId=null; saveState(); render(); };
  ...
  wrap.appendChild(line); wrap.appendChild(score); wrap.appendChild(shots);
  root.appendChild(wrap);

  if (showSettings) renderSettingsPanel();
}

//////////////////// SETTINGS (vč. správy a archivu) ////////////////////
function renderSettingsPanel(){
  ...
  const tBtn=document.createElement("button"); tBtn.textContent= tilesCompact ? "Kompakt" : "Velké"; tBtn.className="px-3 py-1 rounded "+(tilesCompact?"bg-gray-600":"bg-gray-700");
  tBtn.onclick=()=>{ tilesCompact=!tilesCompact; saveState(); render(); };
...
function addManualPlayer(cislo, jmeno, typ, petka){
  ...
  const full=`${String(cislo||"").trim()} ${String(jmeno||"").trim()}`.trim();
  const t=(typ||"").toUpperCase();
  if(!full || !["B","O","Ú"].includes(t)){ alert("Zkontroluj číslo/jméno a typ (B/O/Ú)."); return; }
  hraci.push({id, jmeno:full, typ:t, petka:Number(petka)||0, active:true});
  initStatsFor(id); saveState(); render();
}
function softDeletePlayer(id){
  checkpoint(); const h=hById(id); if(!h) return;
  if(!confirm(`Deaktivovat hráče ${h.jmeno}? Události zůstanou.`)) return;
  h.active=false; saveState(); render();
}
function restorePlayer(id){ checkpoint(); const h=hById(id); if(!h) return; h.active=true; saveState(); render(); }
...
function editPlayerNumber(id){
  const h=hById(id); if(!h) return;
  const cur = cisloZJmena(h.jmeno);
  const nov = prompt(`Nové číslo hráče ${h.jmeno}`, cur);
  if(nov===null) return;
  const name = jmenoBezCisla(h.jmeno);
...
function renderRosterAdmin(container){
  ...
  const [wNum,inNum]=mk("Číslo","text",{placeholder:"12"});
...
function seznamPeticKZobrazeni(){
  const set=new Set();
  hraci.forEach(h=>{ if(h.active!==false && h.typ!=="B" && h.petka>0) set.add(h.petka); });
...
function renderHraci(){
  ...
  if(aktivniPetka>0){
    const g=list.filter(h=>h.typ==="B");
    const p=list.filter(h=>h.typ!=="B" && h.petka===aktivniPetka);
    list=[...g,...p];
  }
  list.forEach(h=>{
    const b=document.createElement("button");
...
function renderStatistiky(){
  ...
  const h2=document.createElement("h2"); h2.className="text-xl font-bold mb-2"; h2.textContent="📊 Statistiky po třetinách"; box.appendChild(h2);
  ...
  sortHraci(hraci).forEach(h=>{
    ...
    tr.appendChild(td(cisloZJmena(h.jmeno)));
    const nameTd=td(h.jmeno); nameTd.style.cursor="pointer"; nameTd.onclick=()=>openPlayerDetail(h.id); tr.appendChild(nameTd);
    tr.appendChild(td(h.typ));
    tr.appendChild(td(h.petka||"-"));

    let total=0;
    const zasPer=["1","2","3","P"].map(t=> h.typ==="B" ? (s.zasahy[t]||0) : 0);
...
function openPlayerDetail(id){
  ...
  const backdrop=document.createElement("div"); backdrop.id="player-detail";
  backdrop.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,.6);display:flex;align-items:center;justify-content:center;z-index:9999;";
  ...
  if(h.typ==="B"){ add("Obdržené góly (časy)", times(s.obdrzene)); add("Zásahy", num(s.zasahy)); }
  else { add("Góly (časy)", times(s.goly)); add("Střely", num(s.strely)); }
  ...
  card.appendChild(grid); backdrop.appendChild(card); document.body.appendChild(backdrop);
}
...
function exportCSVUdalosti(){
  const rows=[]; const add=(obj)=>rows.push(obj);
  // metadata
  ...
  const a=document.createElement("a"); a.href=URL.createObjectURL(blob); a.download="udalosti.csv"; a.click(); URL.revokeObjectURL(a.href);
}
...
function renderArchiv(container){
  ...
  const renderList=()=>{
    ...
    nactiArchiv().filter(e=>{
      ...
      bLoad.onclick=()=>{ if(!confirm("Načíst tento zápas? Přepíše aktuální stav.")) return;
        checkpoint();
        // načti soupisku, resetuj statistiky (nový zápas)
